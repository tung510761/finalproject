...
server <- function(input, output) {
    ...
    output$Plot<-renderPlot({
        ...
        pic_total_per_month <- print(ggplotly(ggplot(df_107) +
    ...
    output$prop<-renderPlot({
      pic_prop <- print(ggplotly(ggplot(prop) +
    ...
    output$plot<-renderPlot({
      print(ggplotly(ggplot(total_107)+geom_bar(aes(x=station,y=n_total),stat="identity")+
        theme(text=element_text(family="BiauKai", size=14))))
